...
startDockerNetwork() {
	if [ -z "$DOCKER_NETWORKED_CHECKED" ]; then
		...
		if ! docker network ls | grep -s "javawebprojectexamplenet"; then
			verbosePrint "Starting docker network javawebprojectexamplenet on 10.241.114.0/24"
			docker network create -d bridge --subnet 10.241.114.0/24 --gateway 10.241.114.1 "javawebprojectexamplenet"
		else
			verbosePrint "Docker network javawebprojectexamplenet already running"
		fi
	fi
}
...
Url: http://localhost:8080/JavaWebProjectExample

Details for components:
javaCode {Source:\"mvn\", Default-Type:\"local\"}
  -t javaCode:local #build local and respect -j
  -t javaCode:docker:[TAG] #docker based build, default tag: latest, uses image https://hub.docker.com/_/maven
...
if [ -z "$1" ]; then
	...
	ALL_COMPONENTS=(JAVACODE MYSQL TOMCAT)
	for comp in "$@"; do
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="d61540d7d18c639a83056e0c18889cc7"
...
verbosePrint "DEFAULT: TYPE_SOURCE_JAVACODE=local"
TYPE_SOURCE_JAVACODE=local
	...
	cat <<-EOF >Vagrantfile
		...
		Vagrant.configure("2") do |config|
		  ...
		  config.vm.provision "shell", inline: <<-SHELL
		    ...
		    apt-get update    
		      ...
		      if [ "\$(cat /etc/*release|grep ^ID=)" = "ID=debian"  ]; then \\
		        if [ "\$(cat /etc/debian_version)" = "8.11" ]; then \\
		            apt-get -qy install maven openjdk-8-jdk-headless mysql-client-5.7 docker.io; \\
		        elif [ "\$(cat /etc/debian_version)" = "9.5" ]; then \\
		           apt-get -qy install maven openjdk-8-jdk-headless mysql-client-5.7 docker.io; \\
		        else  apt-get -qy install maven openjdk-8-jdk-headless mysql-client-5.7 docker.io; fi \\
		    ...
		    echo "...then browse to http://localhost:8080/JavaWebProjectExample"
		  SHELL
		end
	EOF
	vagrant up
...
# MvnPlugin // javaCode
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
verbosePrint "MvnPlugin // javaCode"

if [ "$START_JAVACODE" = "YES" ]; then
	...
	for typeSourceElement in "${array[@]}"; do
		...
		if [ "$comp" == "javacode" ]; then
			TYPE_SOURCE_JAVACODE=$type
			if [ "$TYPE_SOURCE_JAVACODE" == "local" ]; then
				TYPE_SOURCE_JAVACODE_PATH=$pathOrVersion
			else
				TYPE_SOURCE_JAVACODE_VERSION=$pathOrVersion
			fi
		fi

	done

	if [ "$TYPE_SOURCE_JAVACODE" == "docker" ]; then
		if [ -z "$TYPE_SOURCE_JAVACODE_VERSION" ]; then
			TYPE_SOURCE_JAVACODE_VERSION=latest
		fi

	fi

	verbosePrint "TYPE_SOURCE_JAVACODE = $TYPE_SOURCE_JAVACODE // TYPE_SOURCE_JAVACODE_PATH = $TYPE_SOURCE_JAVACODE_PATH // TYPE_SOURCE_JAVACODE_VERSION = $TYPE_SOURCE_JAVACODE_VERSION"
	...
	if [ "$TYPE_SOURCE_JAVACODE" == "docker" ]; then
		...
		f_build() {
			verbosePrint "pwd=$(pwd)\ndocker run --name=javaCode --rm -v $(pwd):/usr/src/build -v "$(pwd)/localrun/.m2":/root/.m2 -w /usr/src/build $dockerImage:$TYPE_SOURCE_JAVACODE_VERSION mvn $MVN_CLEAN $MVN_OPTS package"
...
if [ "$START_MYSQL" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_MYSQL" == "docker" ]; then
		...
		if [ ! -f ".mysqlPid" ]; then
			...
			verbosePrint "docker run --rm -d -p 3306:3306  -e MYSQL_ALLOW_EMPTY_PASSWORD=true   --net=javawebprojectexamplenet --name=mysql   mysql:$TYPE_SOURCE_MYSQL_VERSION"
			dockerContainerIDmysql=$(docker run --rm -d -p 3306:3306 \
				-e MYSQL_ALLOW_EMPTY_PASSWORD=true \
				--net=javawebprojectexamplenet \
	...
	mysql -uroot --protocol=tcp -NB -e "create database if not exists java_code"

	mysql -uroot --protocol=tcp java_code <./src/db/mysql.dump
...
if [ "$START_TOMCAT" = "YES" ]; then
	...
	f_deploy() {
		cp target/JavaWebProjectExample.war $targetPath
	}
	f_deploy
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "download" ]; then
		if [ ! -f ".tomcatPid" ]; then
			...
			REPLVAR_JAVACODE_MYSQL="localhost"

			mkdir -p localrun/7a8013d6

			cat <<EOT7a8013d6 >localrun/7a8013d6/java.properties

{

"db.host": "$REPLVAR_JAVACODE_MYSQL"
...
EOT7a8013d6
			...
			export JAVA_OPTS="-Dconfig.properties=localrun/7a8013d6/java.properties"
	...
	if [ "$TYPE_SOURCE_TOMCAT" == "docker" ]; then
		...
		if [ ! -f ".tomcatPid" ]; then
			...
			if [ "$TYPE_SOURCE_MYSQL" == "docker" ]; then
				REPLVAR_JAVACODE_MYSQL="mysql"
			elif [ "$TYPE_SOURCE_MYSQL" == "local" ]; then
				REPLVAR_JAVACODE_MYSQL="host.docker.internal"
			fi
...
EOT7a8013d6
			...
			verbosePrint "docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080  --net=javawebprojectexamplenet --name=tomcat $ADD_HOST_INTERNAL -v "$(pwd)/localrun/7a8013d6:/tmp/7a8013d6" -e JAVA_OPTS="-Dconfig.properties=/tmp/7a8013d6/java.properties" -v "$(pwd)/localrun/webapps":/usr/local/tomcat/webapps tomcat:$TYPE_SOURCE_TOMCAT_VERSION"
			dockerContainerIDtomcat=$(docker run --rm -d ${dockerAddLibRefs[@]} -p 8080:8080 \
				--net=javawebprojectexamplenet \
				--name=tomcat $ADD_HOST_INTERNAL \
				-v "$(pwd)/localrun/7a8013d6:/tmp/7a8013d6" -e JAVA_OPTS="-Dconfig.properties=/tmp/7a8013d6/java.properties" \
...
if [ "$TAIL" == "YES" ]; then
	echo "http://localhost:8080/JavaWebProjectExample"
